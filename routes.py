...
def analyze_recipe():
    ...
    try:
        ...
        {{
            ...
            "ingredients": [
                "SCAN ENTIRE DOCUMENT for ingredient lists, Zutatenliste, compositions. Include ALL ingredients found. CRITICAL: In Excel/table data with 'Komponentenart' (Component Type) column, ONLY extract rows where Komponentenart is 'COMPONENT' or 'Z_STOP' or similar ingredient types. NEVER extract rows where Komponentenart is 'PRODUCT' - these are the final product itself, NOT ingredients. Example: If row shows 'KnMs 1826 Chococrisp Caramel | PRODUCT | 106%', this is the final product with process loss, DO NOT extract it as an ingredient.",
            ...
            "base_recipe": {{
                "has_base": "CRITICAL: PERFORM NUMERICAL COMPARISON ON REAL INGREDIENTS ONLY. STEP-BY-STEP: (1) Extract ALL ingredient percentages as numbers, but EXCLUDE any rows with Komponentenart='PRODUCT' (these are final products, not ingredients). (2) From the REAL INGREDIENTS ONLY, find the MAXIMUM percentage value. (3) Compare: IF max_percentage > 80 THEN return true, ELSE return false. EXAMPLES: If highest real ingredient is 90% → true (90 > 80), if highest is 55% → false (55 < 80). IGNORE final product rows with 106% or similar. THIS MUST BE A BOOLEAN VALUE (true or false), NOT A STRING.",
                "base_ingredient_name": "IF has_base is true: Extract the NAME of the REAL INGREDIENT (not the final product) that has percentage >80% (translate to English). EXAMPLE: If 'Chococrisp Caramel' ingredient (not product) has 90%, return 'Chococrisp Caramel'. Otherwise: null",
                "base_recipe_number": "IF has_base is true: SEARCH FOR the recipe number associated with the base INGREDIENT (the REAL ingredient with >80% percentage, NOT the final product) using these methods: (1) In PDF tables, check the 'Spezifikation' column (FIRST COLUMN) in the SAME ROW as the high-percentage ingredient - this contains the base recipe number (12-digit pattern like 000001002664). (2) Look for 'Ref.:' or 'Grundrezept:' near the ingredient. (3) Look for 12-digit number patterns in the same row. Extract ONLY the number. Otherwise: null"
            }},
            "nutritional_info": {{
                ...
                if 'ingredients' in recipe_data and isinstance(recipe_data['ingredients'], list):
                    for ingredient in recipe_data['ingredients']:
                        if isinstance(ingredient, dict) and 'recipe_number' in ingredient:
                            ...
                            recipe_number_stripped = recipe_number.lstrip('0')
                            # Check if first non-zero digit is "6"
                            if recipe_number_stripped and recipe_number_stripped[0] == '6':
                                ingredient['status'] = 'unapproved_raw_material'
                                logging.info(f"Marked ingredient '{ingredient.get('name', 'Unknown')}' with recipe number {recipe_number} (first non-zero digit: 6) as unapproved raw material")
...
def publish_recipe():
    ...
    try:
        ...
        if exclusive == 'ja':
            ...
            new_product.is_exclusive = False
        else:
            new_product.is_exclusive = None
        
        # Store department (read-only, from AI extraction)
...
def custom_pages_view(page_id):
    """View a specific custom recipe page with filtering capabilities"""
    init_user_session()
    lang = session.get('language', 'en')
    user_id = session.get('user_id')
